...
class TestPostgresLoader(unittest.TestCase):
    ...
    def test_metadata_file_insert_select_delete(self):
        ...
        try:
            with self.db.transaction() as transaction:
                # insert
                result = transaction.insert_metadata_file(module, uuid, version, example_json)
                self.assertEqual(result, 1)
                # select
                result = transaction.select_metadata_file(module, uuid, version)
                self.assertDictEqual(result['json'], example_json)
        finally:
            with self.db.transaction() as transaction:
                # delete
                result = transaction.delete_metadata_file(module, uuid, version)
                self.assertEqual(result, 1)
                result = transaction.select_metadata_file(module, uuid, version)
                self.assertIsNone(result)
    ...
    def test_table_create_list_delete(self):
        ...
        def test_list(transaction, num_intersecting_tables: int):
            ls_result = set(transaction.select_views())
        ...
        try:
            with self.db.transaction() as transaction:
                ...
                for table_name in test_table_names:
                    transaction.create_metadata_file_view(table_name, module=gen_random_chars(4))
                # list
