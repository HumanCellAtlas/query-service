...
class TestQueryService(unittest.TestCase, DCPAssertMixin):
    ...
    def xtest_create_async_query(self):
    ...
    def xtest_get_job_status_for_uncreated_job(self):
    ...
    def test_get_bundle_fqids_for_file_endpoint(self):
        ...
        query = "SELECT uuid FROM FILES limit 1;"
        file_uuid = self.assertPostResponse(f"{self.api_url}/v1/query",
                                            json_request_body=dict(query=query),
                                            expected_code=requests.codes.ok).json['results'][0]['uuid']
        import pdb
        pdb.set_trace()
    ...
    def xtest_get_file_fqids_for_schema_type_endpoint(self):
