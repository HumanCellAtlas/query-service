...
deploy: init-tf package
	...
	jq 'del(.resource.aws_lambda_function[].filename) | \
            .resource.aws_lambda_function[].s3_bucket="$(TF_S3_BUCKET)" | \
            .resource.aws_lambda_function[].s3_key="$(LAMBDA_SHA).zip" | \
            .resource.aws_lambda_function[].source_code_hash="$(LAMBDA_SHA)" | \
            .resource.aws_api_gateway_deployment.rest_api.lifecycle.create_before_destroy=true' dist/chalice.tf.json > terraform/chalice.tf.json
	terraform apply
	$(MAKE) $(TFSTATE_FILE)
